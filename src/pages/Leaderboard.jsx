    ...
    <div className="leaderboard-page">
      ...
      <div className="lb-table">
        {loading ? (
          ...
          return (
            <div key={row.username} className={`lb-row ${i < 3 ? 'top' : ''}`}>
              ...
              <div className="lb-user">
                ...
                <div className="lb-level">{level.name}</div>
              </div>
              <div className="lb-achievements">
                {(row.achievements || []).slice(0, 3).map(id => {
                  const EMOJIMAP = { perfect_task:'🎯', lightning:'⚡', saver:'🛡️', night_owl:'🌙', marathon:'🔥', no_error_week:'💎' };
                  return <span key={id} className="lb-ach">{EMOJIMAP[id] || '🏅'}</span>;
                })}
              </div>
